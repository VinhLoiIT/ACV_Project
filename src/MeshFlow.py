...
#     x_motion2={}; y_motion2={}
    ...
    cols, rows = width/PIXELS, height/PIXELS

    x_motion = np.zeros((int(rows), int(cols)), dtype=float)
    y_motion = np.zeros((int(rows), int(cols)), dtype=float)

    x_motion2 = np.zeros((int(rows), int(cols)), dtype=float)
    y_motion2 = np.zeros((int(rows), int(cols)), dtype=float)
    ...
    for i in range(height):
        for j in range(width):
            ...
            ptrans=new_points[i*width+j,:]-ptrans
            tmp_matrix[i,j,:]=ptrans
    print("1: ", time() - mark_time)
...
#     radius=300
    ...
    for i in range(int(rows)):
        ...
        i_begin = 0 if (i*PIXELS-radius) < 0 else (i*PIXELS-radius) 
        i_end = height if (i*PIXELS+radius) > height else (i*PIXELS+radius)
        
        for j in range(int(cols)):

            j_begin = 0 if (j*PIXELS-radius) < 0 else (j*PIXELS-radius) 
            j_end = width if (j*PIXELS+radius) > width else j*PIXELS+radius
            
            x_motion2[i][j]=(np.median(tmp_matrix[i_begin:i_end, j_begin:j_end, 0]))
            y_motion2[i][j]=(np.median(tmp_matrix[i_begin:i_end, j_begin:j_end, 1]))
    print("2: ", time() - mark_time)
#     radius=16
#     for i in range(int(rows)):
#         for j in range(int(cols)):
#             tmp_x=(np.median(old_points_matrix[i*radius:(i+1)*radius, j*radius:(j+1)*radius, 0]))
#             tmp_y=(np.median(old_points_matrix[i*radius:(i+1)*radius, j*radius:(j+1)*radius, 1]))
#             pt=[tmp_x, tmp_y]
#             ptrans=point_transform(H,pt)
#             x_motion2[i][j]=new_points_matrix[i*PIXELS,j*PIXELS,0]-pt[0]
#             y_motion2[i][j]=new_points_matrix[i*PIXELS,j*PIXELS,1]-pt[1]
#         #     x_motion2[i][j]=new_points[j*PIXELS + i*PIXELS*width,0]-pt[0]
#         #     y_motion2[i][j]=new_points[j*PIXELS + i*PIXELS*width,1]-pt[1]


    x_motion_mesh = np.zeros((int(rows), int(cols)), dtype=float)
    y_motion_mesh = np.zeros((int(rows), int(cols)), dtype=float)

    x_motion_mesh=x_motion+x_motion2
    y_motion_mesh=y_motion+y_motion2
