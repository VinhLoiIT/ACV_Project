...
# buffer_size=200
def real_time_optimize_path(c, p_pre, p, frame_n, buffer_size=40, iterations=10, window_size=32, beta=1):
    """
    ...
    lambda_t = 100
    # p = np.empty_like(c)
    p_pre_temp=np.empty_like(c[:,:,:frame_n])
    p_temp=np.empty_like(c[:,:,:frame_n])
    ...
    for i in range(c.shape[0]):
        for j in range(c.shape[1]):
            ...
            if (frame_n < buffer_size+1):
                P = np.asarray(c[i, j, :frame_n])
                if not d is None:
                    for _ in range(iterations):
                        alpha = c[i, j, :frame_n] + lambda_t*np.dot(W[:frame_n, :frame_n], P)
                        alpha[:-1] = alpha[:-1] + beta*d
                        gamma = 1 + lambda_t*np.dot(W[:frame_n, :frame_n], np.ones((frame_n,)))
                        gamma[:-1] = gamma[:-1] + beta
                        P = np.divide(alpha, gamma)
            else:
                P = np.asarray(c[i, j, :])
                for _ in range(iterations):
                    alpha = c[i, j, :] + lambda_t*np.dot(W, P)
            ...
            if (frame_n == 1):
                p_temp[i, j, :]=P[-1]
            elif (frame_n < buffer_size+1):
                p_temp[i, j, :] = np.append(p[i,j,:],P[-1])
            else:
                p_temp[i, j, :] = np.append(p[i,j,1:],P[-1])
            
            p_pre_temp[i,j,:]=d

    return p_temp,p_pre_temp
